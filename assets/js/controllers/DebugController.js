angular.module("supertests").controller("DebugController", [
    "$scope",
    "$http",
    function ($scope, $http) {
        $scope.message = "";
        ...
        $scope.countPixels = function ($event) {
            ...
            var scaleY =
                imageCoordinates.height / imageCoordinates.naturalHeight;
        ...
        $scope.enviarTeste = function (guid) {
            if (
                ...
                $http.post("/debug/" + guid + "/enviar").then(success, failure);
            }
        };
    },
]);
